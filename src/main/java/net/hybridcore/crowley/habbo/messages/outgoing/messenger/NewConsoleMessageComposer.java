...
public class NewConsoleMessageComposer extends OutgoingMessage {
    ...
    @Override
    public void run() {
        GameSession target = Crowley.getHabbo().getSessions().getSessionByHabboId(clientMessage.readInt());
        ...
        if (friends) {
            String message = UserInputFilter.filterString(clientMessage.readString());
            //TODO: Log message
